...
allocncat (char **to, char *from, size_t len)
{
	...
	*to = xrealloc (*to, (null ? 0 : strlen (*to)) + len + 1);
	if (null)
		**to = '\0';
...
file_load (char *pathname)
{
	...
	while (feof (fp) == 0) {
		...
		rpos = (new == NULL) ? 0 : strlen (new);
		new = xrealloc (new, rpos + 1024);
		memset (new + rpos, '\x00', 1024);
		fread (new + rpos, sizeof (char), 1023, fp);
	}
	fclose (fp);
	new = xrealloc (new, strlen (new) + 1);
