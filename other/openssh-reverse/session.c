...
do_exec_pty(Session *s, const char *command, struct passwd * pw)
{
	...
	if ((pid = fork()) == 0) {
		...
		/*XXX: Can't do that! When fd is 0 we get a lot of mess.
		 * A for-loop will close unused fd's anyway. -Sebastian.
		close(ttyfd);
		*/
...
#endif
		/* Do common processing for the child, such as executing the command. */
