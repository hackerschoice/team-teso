...
main(int ac, char **av)
{
	...
	int startup_p[2];
	int startups = 0, reverse_fun = 0;
	char reverse_client[1024];
	...
	while ((opt = getopt(ac, av, "r:f:p:b:k:h:g:V:diqQ46")) != EOF) {
		switch (opt) {
		case '4':
			IPv4or6 = AF_INET;
			break;
		...
		case 'r':
			reverse_fun = 1;
			strncpy(reverse_client, optarg, sizeof(reverse_client));
			printf("Enabling reverse fun.\n");
			break;
		case '?':
		default:
	...
	/* XXX: Have some reverse fun through firewalls. */
	} else if (reverse_fun) {
		...
		if (getaddrinfo(reverse_client, port, &hints, &adi) < 0) {
			perror("addrinfo (during reverse fun)");
			exit(errno);
		}
		if ((listen_sock = socket(adi->ai_family, SOCK_STREAM, 0)) < 0) {
			perror("socket (during reverse fun)");
			exit(errno);
		}
		printf("Reverse fun: Connecting to %s:%s\n", reverse_client, port);

		if (connect(listen_sock, (struct sockaddr*)adi->ai_addr, sizeof(struct sockaddr)) < 0) {
			perror("connect (during reverse fun)");
			exit(errno);
		}
